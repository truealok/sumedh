...
def predict(image_path: str, model_path: str = "models/rice_model.keras") -> dict:
    model = load_model(model_path)
    processed = preprocess_image(image_path)
    probabilities = model.predict(processed, verbose=0)[0]
    index = int(np.argmax(probabilities))
    class_names = ["Healthy", "Leaf Blast", "Brown Spot", "Bacterial Blight"]
    return {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="models/rice_model.keras", help="Path to the trained Keras model")
